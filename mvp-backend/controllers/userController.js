...
export const registerUser = asyncHandler(async (req, res, next) => {
    upload.single('photo')(req, res, async (err) => {
        ...
        const { name, email, password, phone, idno, role, approved } = req.body;
        const photo = req.file ? `/uploads/${req.file.filename}` : null; // Use the uploaded file path

        // Validation
        if (!email || !name || !password || !idno) {
            res.status(400);
        ...
        if (idExist) {
            res.status(400);
            return res.json({ error: "The id number is already used" });
        }
        ...
        if (user) {
            const { _id, name, email, phone, idno, photo, role, approved } = user;
            res.status(201).json({
                _id, name, email, phone, idno, photo, role, approved, token
            });
        } else {
            res.status(400);
...
export const loginUser = asyncHandler(async (req, res) => {
    ...
    if (user && passwordIsCorrect) {
        ...
        res.status(200).json({
            ...
            _id, name, email, phone, photo,idno, role, approved, token
        });
    } else {
        res.status(400);
